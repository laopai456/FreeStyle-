# pymem_hw_bp_v4.py
# 方案 B 简化版：直接设置硬件断点，不用调试器附加
#
# 原理：
#   1. OpenProcess + OpenThread 获取线程句柄
#   2. SuspendThread 暂停线程
#   3. GetThreadContext + SetThreadContext 设置 DR
#   4. ResumeThread 恢复线程
#   5. 游戏正常运行，断点自动触发
#
# 注意：需要 sc stop ApolloProtect
...
LOG_FILE = os.path.join(SCRIPT_DIR, f'hwbp_v4_{time.strftime("%Y%m%d_%H%M%S")}.txt')
LOG_F = None
...
def set_hw_bp_direct(pid, tid, addrs):
    """直接设置硬件断点（不用调试器附加）"""
    # 打开线程
    ...
    # 格式: L0-L3 (启用) + R/W0-3 (11=读/写) + LEN0-3 (11=4字节)
    dr7 = 0
...
def main():
    ...
    # 尝试设置每个线程
    success = False
    for t in threads[:5]:  # 只尝试前 5 个线程
        tid = t.id
        log('')
        log(f'尝试线程 {tid}...')
        if set_hw_bp_direct(pid, tid, addrs[:4]):
            success = True
            log(f'✅ 线程 {tid} 设置成功')
            break
        else:
    ...
    log('由于没有调试器，异常会被游戏自己的异常处理器捕获')
    log('可能需要用 DebugActiveProcess 捕获异常')
    log('')
    log('建议：')
    log('1. 如果游戏正常运行，说明断点设置成功但异常被吞了')
    log('2. 如果游戏崩溃，检查崩溃地址')
    log('3. 可以用 WinDbg/x32dbg 附加观察')
