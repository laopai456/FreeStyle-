# pymem_hw_bp_v6.py
# 方案 B 修复版 v6：启用 SeDebugPrivilege + WoW64 API
#
# 关键修复：
#   1. 启用 SeDebugPrivilege 调试权限
#   2. 使用 WoW64 API 访问 32 位上下文
...
LOG_FILE = os.path.join(SCRIPT_DIR, f'hwbp_v6_{time.strftime("%Y%m%d_%H%M%S")}.txt')
LOG_F = None
...
def enable_debug_privilege():
    ...
    if not advapi32.OpenProcessToken(kernel32.GetCurrentProcess(),
                                      ...
                                      ctypes.byref(hToken)):
        log(f'OpenProcessToken 失败: {kernel32.GetLastError()}')
        return False

    # 查找 SeDebugPrivilege 的 LUID
    luid = LUID()
    if not advapi32.LookupPrivilegeValueW(None, "SeDebugPrivilege", ctypes.byref(luid)):
        log(f'LookupPrivilegeValue 失败: {kernel32.GetLastError()}')
    ...
    if not advapi32.AdjustTokenPrivileges(hToken, False, ctypes.byref(tp), 0, None, None):
        log(f'AdjustTokenPrivileges 失败: {kernel32.GetLastError()}')
        kernel32.CloseHandle(hToken)
        return False

    err = kernel32.GetLastError()
    if err == 0:
        log('SeDebugPrivilege 已启用 ✅')
    else:
        log(f'AdjustTokenPrivileges 返回错误: {err}')
...
def set_hw_bp_wow64(pid, tid, addrs):
    ...
    if not ret:
        log(f'Wow64SetThreadContext 返回 FALSE, 错误: {kernel32.GetLastError()}')
        kernel32.ResumeThread(hThread)
        kernel32.CloseHandle(hThread)
        return False

    log(f'Wow64SetThreadContext 返回 TRUE')
...
def main():
    ...
    print('=== 硬件断点 v6 (SeDebugPrivilege + WoW64) ===')
    print(f'SRC_IC: {SRC_IC}')
    print('')

    # 启用调试权限
    log('启用 SeDebugPrivilege...')
    if not enable_debug_privilege():
        log('警告: 无法启用调试权限，可能需要管理员权限')
    print('')
    ...
    if success_tids:
        ...
        log('')
        log('可能原因:')
        log('1. Apollo 仍在干扰（检查 sc query ApolloProtect）')
        log('2. 进程被其他保护机制保护')
        log('3. 需要在游戏启动前设置')
